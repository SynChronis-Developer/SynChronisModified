...
def set_timetable(request):
    if request.method == 'POST':
        try:
            ...
            # Iterate over received timetable data and save or update
            for entry in timetable_entries:
                ...
                TimeTableTable.objects.update_or_create(
                    ClassName=class_obj,
                    day=day,
                    period=period,
                    defaults={
...
class ViewTimetableView(View):
    ...
    def get(self, request, classname):
        class_instance = get_object_or_404(ClassTable, id=classname)
        ...
        for entry in timetable:
            ...
            timetable_data[entry.day][entry.period] = {
                ...
                'endTime': entry.end_time.strftime('%H:%M') if entry.end_time else None
            }
...
class ClassMonthlyAttendanceView(View):
    def get(self, request, class_id, year, month):
        try:
...
# PDF generation view for student attendance
class GenerateStudentPDF(View):
    def get(self, request):
        ...
        for record in attendance:
            p.drawString(100, y, f"Date: {record.Date}, Student: {record.StudentName.StudentName}, Class: {record.StudentName.ClassName.ClassName}, Total Hours: {record.TotalHours}, Attended Hours: {record.AttendedHours}, Status: {record.Status}")
            y -= 20
            if y < 50:
                p.showPage()
                y = 750
...
# PDF generation view for class-wise results
class GenerateClassPDF(View):
    def get(self, request):
        class_id = request.GET.get('class_pdf')
        if not class_id:
        ...
        # Draw things on the PDF. Here's where the PDF generation happens.
        # See the ReportLab documentation for the full list of functionality.
        p.drawString(100, 750, f"Class Attendance Report for Class ID: {class_id}")

        # Fetch data from the database
        attendance = AttendanceTable.objects.filter(StudentName__ClassName=class_id)
        y = 700
        for record in attendance:
            p.drawString(100, y, f"Date: {record.Date}, Student: {record.StudentName.StudentName}, Total Hours: {record.TotalHours}, Attended Hours: {record.AttendedHours}, Status: {record.Status}")
            y -= 20
            if y < 50:
                p.showPage()
                y = 750
        ...
        buffer.close()
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="class_{class_id}_attendance_report.pdf"'
        response.write(pdf)
        return response
...
class ManageSubjectsView(View):
    def get(self, request):
        ...
        semesters = SemesterTable.objects.all()
        subjects = SubjectsTable.objects.all()
        subject_types = SubjectsTable.SType
...
class ClassListView(View):
    def get(self, request):
        teachers = TeacherTable.objects.all()
        semesters = SemesterTable.objects.all()
        classes = ClassTable.objects.select_related().all()
        return render(request, 'class_management.html', {'classes': classes, 'teachers': teachers, 'semesters': semesters})
...
class AddClassView(View):
    def post(self, request):
        ...
        teacher = TeacherTable.objects.get(id=teacher_id) if teacher_id else None
        semester = SemesterTable.objects.get(id=semester_id) if semester_id else None

        new_class = ClassTable.objects.create(
            ClassName=class_name,
            TeacherName=teacher,
            Semester=semester
        )

        return JsonResponse({'message': 'Class added successfully', 'class_id': new_class.id})

class UpdateClassView(View):
    def post(self, request, pk):
        class_instance = get_object_or_404(ClassTable, pk=pk)

        class_instance.ClassName = request.POST.get('ClassName', class_instance.ClassName)
        teacher_id = request.POST.get('TeacherName')
        semester_id = request.POST.get('Semester')

        class_instance.TeacherName = TeacherTable.objects.get(id=teacher_id) if teacher_id else class_instance.TeacherName
        class_instance.Semester = SemesterTable.objects.get(id=semester_id) if semester_id else class_instance.Semester
...
class AssignTeacherView(View):
    ...
    def post(self, request):
        ...
        # Update the teacher's login type to "class teacher"
        teacher_login = teacher_obj.LOGIN
        teacher_login.Type = "class_teacher"
        teacher_login.save()
...
class RemoveTeacherView(View):
    ...
    def delete(self, request):
        try:
            ...
            # Unassign teacher
            teacher_obj = class_obj.TeacherName
            class_obj.TeacherName = None  
            class_obj.save()

            # Revert the teacher's login type to "teacher"
            teacher_login = teacher_obj.LOGIN
